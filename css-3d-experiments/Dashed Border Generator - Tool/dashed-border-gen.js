...
function drawBordr() {
    ...
    const animationProperty = (animation > 0) ? `borderAnimation ${Math.round((1.1 - animation) * 10) / 10}s infinite linear${diraction ? ' reverse' : ''}` : 'unset';
    resultDiv.style.animation = animationProperty;
    ...
    if (animation > 0) {
        ...
        animationTxt = `
                <span><b>animation:</b> ${animationProperty};</span>
            }<br>
            <br>
