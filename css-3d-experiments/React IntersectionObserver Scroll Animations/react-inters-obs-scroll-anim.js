    ...
    React.useEffect(() => {
        ...
        if (!observeRef.current) {
            observeRef.current = new IntersectionObserver(
                    handleIntersection, 
                    {root: observerRef.current, ...options}
                )
        }
...
const App = () => {
    ...
    const [style, setStyle] = React.useState('grid')
        ...
        <div>
            ...
            <div ref={controlsRef} className="controls card elevated filled">
                ...
                {STYLES.map((style, index) => (
                    ...
                    <input defaultChecked={style === 'Grid' ? true : false} type="radio" name="style" value={style.toLowerCase()} id={style.toLowerCase()} />
                    </React.Fragment>
                ))}
                </div>
            </div>
...
ROOT.render(<App/>)
console.log('right after render');